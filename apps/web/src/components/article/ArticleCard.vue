...
<template>
  <article class="card p-5 sm:p-6 hover:shadow-soft transition-all cursor-pointer group"
    :class="{ 'opacity-60': article.isRead }" @click="openArticle">
    <div class="flex gap-4">
      <!-- 封面图 -->
      <div v-if="article.coverImage" class="flex-shrink-0 hidden sm:block">
        <img :src="article.coverImage" :alt="article.title" class="w-28 h-28 object-cover rounded-lg" />
      </div>
      ...
      <div class="flex-1 min-w-0">
        ...
        <!-- 标题 -->
        <h3
          class="text-base font-semibold text-neutral-900 group-hover:text-primary-600 transition-colors mb-2 line-clamp-2">
          {{ article.title }}
        </h3>

        <!-- 摘要 -->
        <p v-if="showSummary !== false && article.summary" class="text-sm text-neutral-600 line-clamp-4 mb-3">
          {{ article.summary }}
        </p>
        ...
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <span v-if="!article.isRead" class="w-2 h-2 bg-primary-500 rounded-full" title="未读"></span>
          ...
          <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <button class="p-1.5 rounded-lg transition-colors"
              ...
              :title="article.isStarred ? '取消收藏' : '收藏'" @click="toggleStar">
              <svg class="w-4 h-4" :fill="article.isStarred ? 'currentColor' : 'none'" stroke="currentColor"
                viewBox="0 0 24 24">
